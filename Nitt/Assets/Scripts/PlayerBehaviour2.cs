    ...
    private void CalcTargetPos()
    {
        ...
        if(hit.collider != null)
        {
            ...
            if(distance < maxTeleportDistance)
            {
                ...
                Vector2 wallOffset = hit.normal * (teleportTarget.transform.localScale/2);
                if(Mathf.Abs(moveDirectionAngle) <= 90)
                {
                    extraOffset = (Quaternion.Euler(0, 0, -90) * hit.normal)*(teleportTarget.transform.localScale.x / 2) / Mathf.Tan(Mathf.Abs(moveDirectionAngle)*Mathf.Deg2Rad);
                }
                else
                {
                    extraOffset = (Quaternion.Euler(0, 0, 90) * hit.normal)*(teleportTarget.transform.localScale.x / 2) / Mathf.Tan((Mathf.Abs(moveDirectionAngle)) * Mathf.Deg2Rad);
                }
                Debug.DrawLine(hit.point, hit.point+extraOffset, Color.cyan);
                Debug.Log(Vector2.Distance(teleportTarget.transform.localScale, hit.normal));
                
                teleportPoint = hit.point + wallOffset + extraOffset;
            }
            else
            {
